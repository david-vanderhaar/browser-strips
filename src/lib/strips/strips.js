...
let StripsManager = {
    ...
    initializeAndGetProblem: function(problem) {
        // Populate list of parameter values.
        var values = {};
        for (var i in problem.states) {
            var state = problem.states[i];
    ...
    loadFromJson: function(domain, problem, callback) {
        problem = StripsManager.initializeAndGetProblem(problem)
        // Give a copy of the possible parameter values to the domain.
        domain.values = problem.values;
    ...
    isPreconditionSatisfied: function(state, precondition) {
        ...
        for (var i = 0; i < precondition.length; i++) {
            ...
            for (var l in state.actions) {
                var match = true;
                let operation = precondition[i].operation || 'and'; // If no operation is specified, default to 'and'. Must explicitly provide 'not' where required.
    ...
    nextGraphLayer: function(domain, parentLayer, isSkipNegativeLiterals) {
        ...
        let lastLiteralCount = layer.length;
        let lastActionCount = actions.length;
        let lastGraphIndex = 0;
